...
export default function FeedPrincipal({ itensLocais }: { itensLocais: any[] }) {
  ...
  useEffect(() => {
    ...
    const combinarTudo = () => {
      ...
      // Junta tudo (Locais + Firebase)
      const todosJuntos = [...fbPosts, ...itensLocais];

      // ✨ FILTRO ANTI-CLONES: Remove duplicatas checando o ID (ou slug)
      // Se vier um do cache e um do Firebase, ele unifica.
    ...
    const unsubMom = onSnapshot(qMomentos, (snap) => {
      momentosFB = snap.docs.map(doc => ({
        id: doc.id,
        tipo: 'momento',
        descricao: doc.data().descricao,
        imagens: doc.data().imagens || [],
        autorNome: doc.data().autorNome,
        autorAvatar: doc.data().autorAvatar,
  ...
  return (
    <div className="columns-2 sm:columns-3 md:columns-4 lg:columns-5 gap-4 sm:gap-6 space-y-4 sm:space-y-6">
      {feedCompleto.map((item) => {
        ...
        // Define para onde o link vai dependendo do tipo
        let linkDestino = '#';
        if (isArtigo) {
              ...
              {isMomento ? (
                <div className="bg-white border-2 border-slate-200 rounded-2xl overflow-hidden shadow-sm hover:shadow-lg hover:border-blue-400 transition-all duration-300 flex flex-col h-full">
                  <div className="p-3 flex items-center gap-2 border-b border-slate-50">
                    <img 
                      src={item.autorAvatar || `https://ui-avatars.com/api/?name=${item.autorNome}`} 
                      alt={item.autorNome} 
                  ...
                  {item.imagens && item.imagens.length > 0 && (
                    <div className="relative w-full overflow-hidden bg-slate-100">
                      ...
                      {item.imagens.length > 1 && (
                        <div className="absolute top-2 right-2 bg-black/60 backdrop-blur-sm text-white px-2 py-1 rounded-full text-[10px] font-bold shadow-sm">
                          + {item.imagens.length - 1} foto{item.imagens.length - 1 > 1 ? 's' : ''}
                        </div>
                      )}
                    </div>
                  )}
                  ...
                  <div className="p-3 md:p-4">
                    <p className="text-[13px] md:text-sm text-slate-600 leading-snug line-clamp-4 break-words">
                      {item.descricao}
                    </p>
                  </div>
                </div>
